...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self._visited, self._visitedlist = {}, [], 0
...
def foodHeuristic(state, problem):
    ...
    source = max((e, k) for k, e in tree[position].items())[1]
    vertices = []
    for node in remaining:
        if node == source:
    ...
    for _ in range(len(vertices) - 1):
        for u in vertices:
            for e in vertices:
                ...
                if u.dist + tree[u.node][e.node] < e.dist:
                    e.dist = u.dist + tree[u.node][e.node]
    ...
    if len(remaining) > 0:
        ...
        if start in remaining:
            return distance(position, start) + initial

        for cost, node in path:
            if position == node: return cost
            if node in remaining:
                return cost + distance(position, node)
    return 0
